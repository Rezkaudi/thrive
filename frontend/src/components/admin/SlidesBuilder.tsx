...
export const SlidesBuilder: React.FC<SlidesBuilderProps> = ({
  ...
  const addInteractiveItem = (slideIndex: number) => {
    const slide = slides[slideIndex];
    const interactiveContent = slide.content.content as InteractiveContent;
    const currentType = interactiveContent?.type || 'drag-drop';
    const newItem = getDefaultInteractiveItem(currentType);
    updateInteractiveContent(slideIndex, {
      items: [...(interactiveContent.items || []), newItem]
    });
  };

  const updateInteractiveItem = (slideIndex: number, itemIndex: number, updates: any) => {
    const slide = slides[slideIndex];
    const interactiveContent = slide.content.content as InteractiveContent;
    const newItems = [...(interactiveContent.items || [])];
    newItems[itemIndex] = { ...newItems[itemIndex], ...updates };
    updateInteractiveContent(slideIndex, { items: newItems });
  };

  const removeInteractiveItem = (slideIndex: number, itemIndex: number) => {
    const slide = slides[slideIndex];
    const interactiveContent = slide.content.content as InteractiveContent;
            ...
            <Card variant="outlined" sx={{ p: 3 }}>
              <Stack direction="row" justifyContent="space-between" alignItems="center" mb={3}>
                ...
                <Button
                  variant="contained"
                  startIcon={<Add />}
                  onClick={() => addInteractiveItem(index)}
                  size="small"
                  sx={{ borderRadius: 2 }}
                >
                  Add Item
                  ...
                  {interactiveContent.items.map((item: any, itemIndex: number) => (
                    <Card key={item.id || itemIndex} variant="outlined" sx={{
                      ...
                      <CardContent>
                          ...
                          <Stack direction="row" spacing={1}>
                            ...
                            <Tooltip title="Delete Item">
                              <IconButton
                                size="small"
                                color="error"
                                onClick={() => removeInteractiveItem(index, itemIndex)}
                              >
                        ...
                        {renderInteractiveItemEditor(currentType, item, itemIndex, (idx: number, updates: any) => 
                          updateInteractiveItem(index, idx, updates)
                        )}
                      </CardContent>
                    </Card>
                  ))}
