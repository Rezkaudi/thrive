...
import { LessonFormState, LessonType, Keyword } from "../../types/lsesson-form.types";
...
export const ContentForm: React.FC<ContentFormProps> = ({
  ...
  useEffect(() => {
    if (lessonType === "KEYWORDS") {
      const current = watch("keywords") as Keyword[] | null | undefined;
      if (!Array.isArray(current)) {
        setValue("keywords", [], { shouldDirty: false, shouldValidate: false });
      }
    } else if (lessonType === "QUIZ") {
      const cd = watch("contentData") as any;
      if (!cd || !Array.isArray(cd.questions)) {
        setValue("contentData", { questions: [] }, { shouldDirty: false, shouldValidate: false });
      }
    } else if (lessonType === "SLIDES") {
      const cd = watch("contentData") as any;
      if (!cd || !Array.isArray(cd.slides)) {
        setValue("contentData", { slides: [] }, { shouldDirty: false, shouldValidate: false });
      }
    } else {
      const url = watch("contentUrl");
      if (typeof url !== "string") {
        setValue("contentUrl", "", { shouldDirty: false, shouldValidate: false });
      }
    }
          ...
          render={({ field }) => {
            ...
            return (
              <QuizBuilder
                ...
                initialQuestions={Array.isArray(value.questions) ? value.questions : []}
                passingScore={(editingLesson?.passingScore as number) ?? undefined}
          ...
          render={({ field }) => {
            ...
            return (
              <SlidesBuilder
                ...
                initialSlides={Array.isArray(value.slides) ? value.slides : []}
                onChange={(slides) => {
                  setValue(
                    "contentData",
                    { slides: slides ?? [] },
